...
const AttendeeLandingPage: React.FC = () => {
  ...
  }, [page, query]); // Added query here

  const refreshPublishedEvents = async () => {
    try {
      const events = await listPublishedEvents(page);
  ...
  const queryPublishedEvents = async () => {
    if (!query || query.trim() === "") {
      return await refreshPublishedEvents(); // Added return
    }

    try {
      const events = await searchPublishedEvents(query, page);
      setPublishedEvents(events);
    } catch (err) {
      handleError(err);
    }
    ...
    <div className="bg-black min-h-screen text-white">
      ...
      <div className="flex justify-end p-4 container mx-auto">
        {isAuthenticated ? (
          <div className="flex gap-4">
            ...
            <Button onClick={() => signoutRedirect()} className="cursor-pointer">
              Log out
            </Button>
          </div>
        ) : (
          <div className="flex gap-4">
            <Button onClick={() => signinRedirect()} className="cursor-pointer">
              Log in
            </Button>
          </div>
        )}
      ...
      <div className="container mx-auto px-4 mb-8">
        <div className="bg-[url(/organizers-landing-hero.png)] bg-cover min-h-[200px] rounded-lg bg-bottom md:min-h-[250px]">
          <div className="bg-black/45 min-h-[200px] md:min-h-[250px] p-15 md:p-20">
            <h1 className="text-2xl font-bold mb-4">Find Tickets to Your Next Event</h1>
      ...
      <div className="grid grid-cols-2 gap-4 px-4 md:grid-cols-4">
        {publishedEvents?.content?.map((publishedEvent) => (
          <PublishedEventCard key={publishedEvent.id} publishedEvent={publishedEvent} />
        ))}
      </div>
